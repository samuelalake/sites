...
import { imgVector } from "@/imports/svg-t0zsn";
      ...
      <div className="max-w-[1536px] mx-auto px-24">
        <div className="border-x border-[rgba(0,0,0,0.06)] px-[49px] py-24">
          ...
          <h2 className="font-['displayFont:Regular','Plus_Jakarta_Sans',sans-serif] text-[44px] leading-[48px] text-[#090b0c] text-center">
            ******* enough to use real data.
          </h2>
          <p className="font-['Geist:Regular',sans-serif] text-[18px] leading-[28px] text-[#737373] text-center mt-6 max-w-2xl mx-auto">
            We built Aside so you never have to choose between powerful AI and keeping your data private.
          </p>
        </div>
          ...
          <div className="flex gap-6 px-px">
            {tiles.map(({ img, title, desc }) => (
              <div key={title} className="flex-1 flex flex-col">
                <div className="bg-[#f5f5f5] h-[320px] rounded-[11.2px] overflow-hidden flex items-center justify-center">
                  {img ? (
                    <img src={img} alt={title} className="size-full object-cover" />
                  ) : (
                    <img src={imgVector} alt="App logos" className="max-h-full max-w-full object-contain" />
                  )}
                </div>
